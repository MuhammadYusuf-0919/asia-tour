...
const BestResults: React.FC = () => {
  ...
  return (
    <div className="container-lg grid py-[50px] md:py-[35px] sm:py-[20px] gap-y-[40px] md:gap-y-[30px] sm:gap-y-[20px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-1 justify-between gap-x-[80px] md:gap-x-[60px] md:gap-y-[50px]">
        <div className="grid gap-y-[80px] md:gap-y-[60px] sm:gap-y-[40px]">
          ...
          <Card>
            ...
            <div className="grid grid-cols-4 py-[20px] px-[25px] gap-[20px]">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="contents">
            <div className="h-full p-[25px] grid gap-y-[20px]">
              ...
              <Select
                ...
                errorMessage={errors.ticketSelect && "Please select a ticket"}
              >
...
{
  /* <Card>
          <BlockHeader title='Book this Tour' />
          <div className="h-full py-[35px] px-[25px] grid grid-cols-2 gap-x-[60px] gap-y-[35px] mdgap-x-[45px] mdgap-y-[20px] sm:gap-x-[30px] sm:gap-y-[18px]">
            {bestResultsData.map(({ icon, title, desc }) => (
              <div className="flex items-center" key={title}>
                <img src={icon} alt={`${title}-image`} />
                <div>
                  <h4 className="text-dark font-roboto text-2xl font-medium">
                    {title}
                  </h4>
                  <p className="text-grayDark font-roboto text-lg">{desc}</p>
                </div>
              </div>
            ))}
          </div>
